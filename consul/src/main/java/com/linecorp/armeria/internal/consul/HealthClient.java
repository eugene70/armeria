...
final class HealthClient {
    ...
    private CompletableFuture<List<HealthService>> service(String serviceName, QueryParams params) {
        ...
        return client.consulWebClient()
                     ...
                     .aggregate()
                     .thenApply(response -> {
                         final List<HealthService> healths = new ArrayList<>();
                         logger.trace("response: {}", response);
                         if (!Objects.equals(response.status(), HttpStatus.OK)) {
                             return healths;
                         }
                         try {
    ...
    public CompletableFuture<List<Endpoint>> endpoints(String serviceName, QueryParams params) {
        return service(serviceName, params).thenApply(
                nodes ->
                        nodes.stream()
                             .map(node -> {
                                 final String host;
    ...
    private static class HealthService {
        @JsonProperty("Node")
        Node node;
